...
%if numiter/10 there is reminder? if not maximize, if yes keep going
%% Parameters
beta = 0.987;
mu = 2;
alpha = 1/3;
...
c = zeros(length(nk), length(nz));
V0 = zeros(length(nk),length(nz));
V = zeros(length(nk),length(nz));
u = zeros(length(nk),length(nz));
tic;
...
title("Value function iteration - Accelerator")
accelerator=gcf
saveas(accelerator, 'accelerator.png')
...
policy_acc=gcf
saveas(policy_acc, 'policy_acc.png')
%%
